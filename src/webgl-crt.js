...
let drawLine = 0;
let lastRefresh = 0;
let txtScreenContent = new Array(32 * 16).fill('@');
let txtScreenColours = new Array(32 * 16).fill('txtGGreen');

function getColour(className) {
  let colour = '#20c020';
  ...
  // console.log(className, colour);

  // Additional code to apply the colour can be here.
  return colour;
}
...
function renderWorld(delta) {
  ...
  //position top left
  // bufferContext.translate(consolePad, consolePad);
  
  
  
  if (lastRefresh < performance.now() - 20) {
    lastRefresh = performance.now();
    let curChar = 0
    for (let col = 0; col < 32; col ++) {
      const char = txtScreenContent[drawLine * 32 + col];
      // const char = String.fromCharCode(idx);
      // bufferContext.fillStyle = '#0F0';
      const colour = getColour(txtScreenColours[drawLine * 32 + col]);
      if (colour == '#000000') {
        bufferContext.fillStyle = '#0F0';
        bufferContext.fillRect(
          col * (charW+8) + 8, // center inside character box
          drawLine * (charH+4) + 4,
          charW+8,
          charH+3,
        )
      ...
      bufferContext.fillText(
        char,
        col * (charW+8) + 15, // center inside character box
        drawLine * (charH+4) + 15,
      );
      curChar++;
    }
    drawLine = (drawLine + 1) % 16;
  }
    ...
    frag: `
        ...
        void main() {
            ...
            if (fromEdge.x > 0.0 && fromEdge.y > 0.0) {
                ...
                float scanlineAmount = inTexelOffset.y * inTexelOffset.y / 0.1;
                float intensity = 7.0 - scanlineAmount * 5.0 + glitchOffset * 2.0; // ray intensity is over-amped by default
                ...
                gl_FragColor = vec4(edgeFade * screenFade * mix(
                    bgColor,
                    fgColor,
                    intensity * pixelRGB + glitchOffset * 0.3
...
function rafBody() {
  ...
  regl.poll();
  spriteTexture.subimage(bufferContext, 0, 0);
  quadCommand({
      bgColor: termBgColor,
      fgColor: termFgColor
  });
